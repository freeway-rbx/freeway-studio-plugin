...
function InstanceWirerComponent:onClickSyncButton() end

function InstanceWirerComponent:didMount() end

function InstanceWirerComponent:willUnmount()
	--self:onClickDisconnectButton()
end

function InstanceWirerComponent:init() end
...
function InstanceWirerComponent:renderPropertyWires(i)
	...
	if self.props.piece ~= nil and self.props.piece.type ~= "image" then
		content = nil
	end

	if #self.props.instances > 1 then
		wireLabel = "Wire All"
		unwireLabel = "Unwire All"
	end
